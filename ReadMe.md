...
```
*Note the domain `id` returned, which is required for all subsequent steps.*
...
```
*If verification fails, double-check that the TXT record has propagated globally (e.g., using `dig txt _acme-challenge.example.com`).*
...
```
*The service is now communicating with the ACME CA (e.g. Let's Encrypt) in the background. You must update your TXT record to match the returned `"expected_txt_value"` so the ACME CA can complete the verification challenge.*
...
```
*The status changes to `active` once the certificate is successfully issued and stored in the database.*
